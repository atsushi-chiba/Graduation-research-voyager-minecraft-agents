...
6. **tickrate**: サーバー再起動で 1x に戻るので、起動確認したら**必ず** `curl -s -X POST 'http://localhost:8089/tickrate?multiplier=10'` で 10x に戻す(ユーザー指示 2026-07-02: シミュレーションは常に10倍速で回す)。tickrate は bridge の HTTP エンドポイントであり、コンソールコマンドではない — cmd_pipe に送ると `tickrate 10<--[HERE]` という不明コマンドエラーになる(`<--[HERE]` 付きの応答はエラーであって成功ではない)。mod 側でキック/ログインタイムアウト抑止済みなのでリアルプレイヤーがいても 10x のままでよい。
